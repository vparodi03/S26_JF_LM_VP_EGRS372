...
// CLASS: EGRS372
// PROJECT: Lab 3
// PURPOSE: Create a program to control the servo motors on a TurtleBot using feedback from the Transformation Matrix and quaternion in ROS
...
void Turnprog(const tf::tfMessage cvalue)
{
  ...
  {
      target_angle =  current_angle - (2*PI/sides);
  }
  ...
  if((current_angle<=target_angle)&&(std::abs(current_angle-target_angle)<0.1)&&(sides<=10))
  {
    flag = false;
  }
  ...
  target_speed = mindist/PI+target_forward;
}


// MAIN PROGRAM - TAKES NUMBER OF SIDES AND DISTANCE LENGTH FROM USER AND PERFORM SELECTED SHAPE
int main(int argc, char **argv)
{
  ...
  // GETS A SIDE NUMBER FROM THE USER
  fflush(stdin);
  std::cout << "Enter number of sides to the shape: "; // Ask the user for the number of sides
  // While loop to get the number of sides until it is an integer bigger than 1
  while(sides<1 && ros::ok())
  {
    std::cin >> sides;
  }

  // GETS A SIDE LENGTH FROM THE USER
  fflush(stdin);
  std::cout << "Enter side length: "; // Ask the user for the legth of the sides
  // While loop to get the number of sides until it is between 0.1 an 1
  while((target_forward<0.1 || target_forward>1.0) && ros::ok())
  {
  ...
  // IF NUMBER OF SIDES ENTERED IS LESS OR EQUAL TO 10 - PERFORM CORRESPONDING SHAPE
  if (sides<=10)
  {
    // LOOPS DEPENDING ON THE NUMBER OF SIDES (FOR EACH SIDE OF A SHAPE)
	...
	//INVOKES ALL SUBSCRIBERS CALLBACKFUNCTIONS
      ...
      target_angle= target_angle -(2*PI/sides);
  ...
  // IF NUMBER OF SIDES IS BIGGER THAN 10 - PERFORM A CIRCLE SHAPE
  else
  {
    ...
    // DECLARE SUBSCRIBER TO THE TF TOPIC  WITH THE FUNCTION Turnprog
    tf = n.subscribe("/tf", 1, Turnprog);
    ros::spinOnce();
    loop_rate.sleep();

    prev_angle = current_angle; // Record the original position of the robot
    track_angle = 0.0; // Set the tracking angle to 0.0

    // GOES FORWARD UNTIL THE NECESSARY DISTANCE IS TRAVELED
    while((track_angle < (2*PI-0.03)) && ros::ok())
    {
      // PUBLISHES THE TARGET SPEED, CALCULATED WHEN THE ROBOT POSITION IS SUBSCRIBED (WHEN ros::spinOnce(); IS RUN)
      c.linear.x=0.2; // Constant linear velocity
      c.angular.z=-0.2/target_forward; // Angular velocity - Clockwise direction

      track_angle=current_angle;  // Set track angle to current angle
      vel_pub.publish(c);
      ...
      // Declare increment to monitor the rotation angle
      double increment = current_angle - prev_angle;
      // If statement to maintain the angle between 0and 2PI - adjust by 2PI
      if (increment < -PI)
      {
        increment += 2*PI;
      }
      ...
      // Increment the tracking angle
      track_angle += std::abs(increment);
